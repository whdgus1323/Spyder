...
for cat in categories:
    if cat not in junction_grid_map or cat not in prec_mean_map:
        continue

    counts = junction_grid_map[cat]
    data_text = prec_mean_map[cat]

    plt.figure(figsize=(6, 6))
    plt.imshow(counts, origin="lower", vmin=0, vmax=global_junc_max)
    plt.title(f"{cat} junction count (heat) / precursor mean (text)")
    plt.xlabel("grid col")
    plt.ylabel("grid row")
    plt.colorbar()
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            t = plt.text(c, r, txt, ha="center", va="center", color=color)
            t.set_path_effects([
